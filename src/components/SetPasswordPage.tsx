...
export default function SetPasswordPage({ onDone }: { onDone: () => void }) {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    color: 'var(--text-1)', fontFamily: 'inherit', outline: 'none', width: '100%',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '32px 32px 24px', borderBottom: '1px solid var(--border)', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 600, letterSpacing: '-0.01em' }}>Welcome</div>
          <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 4 }}>Set a password to activate your account</div>
        </div>

        <form onSubmit={handleSubmit} style={{ padding: '24px 32px 32px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-2)' }}>New password</span>
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-2)' }}>Confirm password</span>
