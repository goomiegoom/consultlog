// Shared UI atoms — icons, progress bar, modal, button, etc.
// Dark, minimal, Claude-inspired warm-orange accent.

// ── Icons (1.5px stroke, 16x16) ─────────────────────────────────────────────
const Icon = ({ d, size = 16, fill = "none", stroke = "currentColor", strokeWidth = 1.5 }) => (
...
const IconClose      = (p) => <Icon {...p} d="M4 4l8 8M12 4l-8 8" />;
const IconClock      = (p) => (
...
const IconCheck      = (p) => <Icon {...p} d="M3 8.5l3 3 7-7" />;
const IconChevDown   = (p) => <Icon {...p} d="M4 6l4 4 4-4" />;
const IconChevRight  = (p) => <Icon {...p} d="M6 4l4 4-4 4" />;
const IconArrowRight = (p) => <Icon {...p} d="M3 8h10M9 4l4 4-4 4" />;
const IconEdit       = (p) => <Icon {...p} d="M11 2.5l2.5 2.5L6 12.5H3.5V10z" />;
const IconTrash      = (p) => (
  <svg width={p.size||16} height={p.size||16} viewBox="0 0 16 16" fill="none"
       stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
// ── Avatar (initials) ───────────────────────────────────────────────────────
function Avatar({ name, size = 24, tone = "default" }) {
  const initials = name
...
// ── Progress bar ────────────────────────────────────────────────────────────
// Turns warm at 80%+, red at 100%+. Shows overage as a separate bar segment.
function ProgressBar({ used, included, height = 8, showLabels = false }) {
  ...
  let color = "var(--accent)"; // default warm orange
...
// ── Button ──────────────────────────────────────────────────────────────────
function Button({ children, onClick, variant = "ghost", size = "md", icon, disabled, type = "button" }) {
  const styles = {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled}
      ...
      style={{
        ...
        background: hov && !disabled ? styles.hov : styles.bg,
        color: styles.fg,
        border: `1px solid ${styles.border === "transparent" ? "transparent" : styles.border}`,
...
function statusForUsage({ pct, overHours }) {
  if (overHours > 0) return { tone: "over", label: "Over budget" };
  if (pct >= 80) return { tone: "warn", label: "Near limit" };
  return { tone: "ok", label: "On track" };
}

Object.assign(window, {
  IconPlus, IconClose, IconClock, IconUser, IconBuilding, IconCalendar,
  IconCheck, IconChevDown, IconChevRight, IconArrowRight, IconEdit,
  IconTrash, IconWarning, IconSparkle,
