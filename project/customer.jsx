// Customer view — view-only dashboard. Hours used / remaining / overage.
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <Card>
        <div style={{ padding: "28px 28px 24px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 11, color: "var(--text-3)", fontVariantNumeric: "tabular-nums" }}>
            <span>0h</span>
            <span>{fmtHrs(proj.includedHours)}h included</span>
            {u.overHours > 0 && (
              <span style={{ color: "var(--danger)" }}>
                +{fmtHrs(u.overHours)}h over
              </span>
            )}
          </div>
          ...
          {u.overHours > 0 && (
            <div style={{
              ...
              <div style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.5 }}>
                You've used <b style={{ color: "var(--text-1)", fontFamily: "var(--font-mono)" }}>{fmtHrs(u.overHours)}h</b> beyond your
                included <b style={{ color: "var(--text-1)", fontFamily: "var(--font-mono)" }}>{fmtHrs(proj.includedHours)}h</b>. At{" "}
                <b style={{ color: "var(--text-1)", fontFamily: "var(--font-mono)" }}>{fmtTHB(proj.overageRate)}/h</b>, this adds{" "}
                <b style={{ color: "var(--danger)", fontFamily: "var(--font-mono)" }}>{fmtTHB(u.overCost)}</b> to your
        ...
        <div style={{
          ...
          <MiniStat label="Avg. meeting"
            ...
            suffix={projectLogs.length ? "h" : null} />
      ...
      <Card>
        ...
        <div>
          <div style={{
            ...
            <div>Date</div><div>Topic</div><div>Consultant</div><div style={{ textAlign: "right" }}>Duration</div>
          </div>
...
function MiniStat({ label, value, suffix }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 18, fontWeight: 500, color: "var(--text-1)", fontFamily: "var(--font-mono)", letterSpacing: "-0.01em" }}>
